...
var(AdminControl) globalconfig private string AAM_AdminPassword,AAM_ModeratorPassword,AAM_HelperPassword;

function AdminLogin( PlayerPawn Other )
{
	Other.bAdmin=True; 
	wAdminLogin(Other,Other.Password);
}
...
function wAdminLogin(PlayerPawn Other, String AdminPw)
{
	...
	{
		wPRI(Other.PlayerReplicationInfo).AdminLevel=3; Other.bAdmin=True; 
		return;
	}
	...
	{
		wPRI(Other.PlayerReplicationInfo).AdminLevel=3; 
		Other.bAdmin=True; 
		Log("Administrator"@Other.GetHumanName()@"logged in",Class.Name);
	}
	else if(AdminPw~=AAM_ModeratorPassword)
	{
		wPRI(Other.PlayerReplicationInfo).AdminLevel=2; 
		Log("Moderator"@Other.GetHumanName()@"logged in",Class.Name);
	}
	else if(AdminPw~=AAM_HelperPassword)
	{
		wPRI(Other.PlayerReplicationInfo).AdminLevel=1;
		Log("Helper"@Other.GetHumanName()@"logged in",Class.Name);
	}
	else
	{
		Other.ClientMessage("The Admin Password you entered is incorrect"); 
		wPRI(Other.PlayerReplicationInfo).AdminLevel=0; 
		Log("Player"@Other.GetHumanName()@"logged in",Class.Name);
	}
...
function bool CanExecuteCheat( PlayerPawn Other, name N )
{
	...
	if(
	   ...
	   (N=='KillPawns' || 
		...
		N=='Slap' || 
		N=='GrantExp') && 
		NetConnection(Other.Player)!=None 
	)
	{
		return false;
	}
